...
function * login (action) {
  try {
    ...
    yield put({ type: authActions.LOGIN_USER_ERROR, payload: error.response.data.message })
  }
}

function * create (action) {
  try {
    const response = yield call(services.auth.create, action.payload)
    services.cookies.set('token', response.data.apiKey);
    history.push('/');
    yield put({ type: authActions.CREATE_USER_SUCCESS, payload: response.data})
  } catch (error) {
    yield put({ type: authActions.CREATE_USER_ERROR, payload: error.response.data.message })
  }
}
...
function * logout () {
  try {
    ...
    yield put({ type: authActions.LOGOUT_USER_ERROR, payload: error.response.data.message })
  }
}
